...
def pobierz_ogloszenia():
    """
    Pobiera ogloszenia z ostatnich DNI_WSTECZ dni, strona po stronie.

    # >>> DOPASUJ <<<
    Ponizsze nazwy parametrow (PublicationDateFrom, PageNumber...) to typowy
    schemat. Jesli inspect_api pokaze inne nazwy - popraw je tutaj.
    """
    ...
    while True:
        params = {
            "PublicationDateFrom": od,   # >>> DOPASUJ jesli inna nazwa
            "PageNumber": strona,
            "PageSize": 100,
        }
        r = requests.get(f"{API_BASE}/notice", params=params, timeout=30)
        if r.status_code != 200:
            print(f"Blad API (strona {strona}): {r.status_code}")
            break
        dane = r.json()

        # >>> DOPASUJ <<< - lista ogloszen bywa pod kluczem np. "value",
        # "items", "data" albo wprost jako lista. inspect_api to pokaze.
        pozycje = dane.get("value") or dane.get("items") or dane.get("data") or []
        ...
        if len(pozycje) < 100:   # ostatnia strona
            break
        strona += 1
...
def link_pdf(ogloszenie):
    nid = ogloszenie.get("noticeId") or ogloszenie.get("id") or ""
    return f"{API_BASE}/Board/GetNoticePdfById?noticeId={nid}"
